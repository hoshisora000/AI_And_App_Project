...
if ($result = mysqli_query($link, "SELECT * FROM winning_numbers")) {
      while ($row = mysqli_fetch_assoc($result)) {
          ...
          $a['data'][] = array($row["period"],$row["super_special"],$row["special"], $row["head1"], $row["head2"], $row["head3"],"<button class='m_btn_delete btn btn-danger btn-xs' id='m_btn_delete'><i class='glyphicon glyphicon-pencil'></i>刪除</button>");
    }
